...
public class SessionRunnerAudioE2ETests : IAsyncLifetime
{
    ...
    public async Task SingleOtherUtterance_ProducesOneTurn()
    {
        ...
        await runner.StartAsync(session.Id, Devices, new TranscriptionOptions(WhisperModelSize.Base, "en", new HashSet<string>()), AudioSourceMode.Both);
    ...
    public async Task MicrophoneOnly_DropsOtherLoopbackUtterance_ProducesNoTurn()
    {
        ...
        var runner = NewRunner(script);
        await runner.StartAsync(session.Id, Devices, new TranscriptionOptions(WhisperModelSize.Base, "en", new HashSet<string>()), AudioSourceMode.MicrophoneOnly);
